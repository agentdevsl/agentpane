...
export class EventProcessingService {
  ...
  async processIncomingEvent(
    ...
    return this.processEventPipeline(source, event, plugin);
  }

  /**
   * Process a scheduled (cron) event that has already been parsed by the scheduler.
   *
   * Skips slug lookup, signature verification, and plugin parsing because those
  ...
  /**
   * Shared event processing pipeline used by both incoming webhooks and scheduled events.
   *
   ...
   */
  private async processEventPipeline(
    source: EventSource,
    event: NormalizedEvent,
    plugin: import('../lib/events/plugin-interface.js').EventSourcePlugin
  ): Promise<Result<ProcessingResult, AppError>> {
    ...
    for (const subscription of matchingSubscriptions) {
      ...
      if (taskResult.ok) {
        ...
        // If targetColumn is 'in_progress', moveColumn triggers agent auto-start.
