...
export class TerraformComposeService {
  ...
  private cleanupSessions(): void {
    ...
    for (const [id, session] of this.sessions) {
      if (now - session.lastAccessedAt > SESSION_TTL_MS) {
        ...
        this.durableStreamsService?.deleteStream(`terraform:${id}`).catch((err) => { log.debug('Failed to delete stream during cleanup', { error: err }); });
      }
    }
    ...
    if (this.sessions.size > MAX_SESSIONS) {
      ...
      for (const [id] of toRemove) {
        this.sessions.delete(id);
        this.durableStreamsService?.deleteStream(`terraform:${id}`).catch((err) => { log.debug('Failed to delete stream during eviction', { error: err }); });
      }
    }
  }
  ...
  async startCompose(
    ...
    try {
      await this.durableStreamsService.deleteStream(streamId).catch((err) => { log.debug('Failed to delete stream before recreate', { error: err }); });
    ...
    this.runPipeline(sid, messages, registryId, composeMode).catch(async (pipelineErr) => {
      ...
      try {
        ...
        await this.durableStreamsService?.deleteStream(`terraform:${sid}`).catch((err) => { log.debug('Failed to delete stream after pipeline error', { error: err }); });
      }
    });
  ...
  private async runPipeline(
    ...
    try {
      ...
      try {
        ...
        // Ensure stream is cleaned up even if error publish fails
        await this.durableStreamsService?.deleteStream(`terraform:${sid}`).catch((err) => { log.debug('Failed to delete stream after publish error', { error: err }); });
      }
    } finally {
      if (session) {
        try {
  ...
  resetSession(sessionId: string): void {
    ...
    this.durableStreamsService?.deleteStream(`terraform:${sessionId}`).catch((err) => { log.debug('Failed to delete stream on session reset', { error: err }); });
  }
