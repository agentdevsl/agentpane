      ...
      >
        ...
        <div
          className={cn(
            ...
            expanded ? 'justify-between px-3.5 py-3' : 'justify-center py-3'
          )}
        >
          {expanded ? (
            <>
              <h2 className="text-[13px] font-semibold text-fg tracking-tight">AgentPane</h2>
              <button
                type="button"
                onClick={toggleFolderPanel}
                className="flex h-5 w-5 items-center justify-center rounded-full border border-border bg-surface text-fg-muted transition-all duration-[220ms] ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-surface-subtle hover:text-fg hover:border-fg-subtle"
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {expanded && (
            <div className="px-3.5 pt-2.5 pb-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-fg-subtle">
              Projects
            </div>
          )}

          {/* Folder list */}
          <div className={cn('py-1', expanded ? 'px-2' : 'px-1 pt-2')}>
            {folders.map((folder) => {
        ...
        <div className="border-t border-border-subtle p-2">
          {expanded ? (
            <button
              ...
              className="flex w-full items-center gap-1.5 rounded-md border border-dashed border-border px-2 py-[7px] text-[12px] font-medium text-fg-subtle transition-all duration-[220ms] ease-[cubic-bezier(0.4,0,0.2,1)] hover:border-accent hover:text-accent hover:bg-accent/5"
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  onClick={() => setCreateDialogOpen(true)}
                  className="flex w-full items-center justify-center rounded-md py-1.5 text-fg-subtle transition-all duration-[220ms] ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-surface-subtle hover:text-accent"
                  data-testid="new-folder-btn"
                >
                  <Plus size={16} />
                </button>
