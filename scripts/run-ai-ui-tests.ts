#!/usr/bin/env bun
/**
 * AI UI Test Runner Script
 *
 * Starts the dev server and runs AI-driven UI tests with JSON output.
 *
 * Usage:
 *   bun scripts/run-ai-ui-tests.ts              # Run all AI UI tests
 *   bun scripts/run-ai-ui-tests.ts --headed     # Run with visible browser
 *   bun scripts/run-ai-ui-tests.ts <pattern>    # Run tests matching pattern
 *   bun scripts/run-ai-ui-tests.ts --parallel   # Enable parallel sessions
 */
...
async function runTests(args: string[]): Promise<number> {
  console.log('🤖 Running AI UI tests with JSON output...\n');

  const testArgs = ['vitest', 'run', '--config', 'vitest.ai-ui.config.ts'];

  // Filter out script-specific args
  const filteredArgs = args.filter(
    (arg) => !arg.includes('run-ai-ui-tests') && arg !== '--headed' && arg !== '--parallel'
  );
  ...
  // Enable parallel sessions if requested
  if (args.includes('--parallel')) {
    env.AI_UI_PARALLEL = 'true';
  }
...
async function main(): Promise<void> {
  ...
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║           AI UI Test Runner (JSON Mode)                    ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');
  ...
  if (exitCode === 0) {
    console.log('\n✅ All AI UI tests passed!');
  } else {
    console.log('\n❌ Some AI UI tests failed');
  }
...
main().catch(async (error) => {
  console.error('AI UI test runner error:', error);
  await cleanup();
  process.exit(1);
});
