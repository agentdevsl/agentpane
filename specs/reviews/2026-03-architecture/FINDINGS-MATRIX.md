...
| SL-004 | Critical | 01-Services | Missing database transactions in multi-step operations | `src/services/agent/agent-execution.service.ts:106-170`, `src/services/task.service.ts:321-410` | Wrap multi-step operations in `db.transaction()` |
| CQ-007 | Critical | 10-Code Quality | container-agent.service.ts is oversized (3,076 LOC) | `src/services/container-agent.service.ts` | Split into lifecycle, plan, worktree, and AgentCore sub-modules |
| CQ-008 | Critical | 10-Code Quality | api.ts God File (1,847 LOC) | `src/server/api.ts` | Extract database init, service factory, and sandbox init into separate modules |
