...
function _mockTxSelectChain(tx: ReturnType<typeof createMockDb>['_mockTx'], rows: unknown[]) {
  const chain = {
    from: vi.fn().mockReturnThis(),
    where: vi.fn().mockReturnThis(),
    leftJoin: vi.fn().mockReturnThis(),
...
function mockTxInsertChain(tx: ReturnType<typeof createMockDb>['_mockTx'], rows: unknown[]) {
  const chain = {
...
function mockUpdateChain(db: ReturnType<typeof createMockDb>, rows: unknown[]) {
  const chain = {
...
describe('GET /tokens', () => {
  ...
  it('pagination returns nextCursor and hasMore when more results exist', async () => {
    ...
    const tokens = [
      makeToken({ id: 'tok-001' }),
      makeToken({ id: 'tok-002' }),
      makeToken({ id: 'tok-003' }),
    ];
...
describe('GET /tokens/:id', () => {
  ...
  it('returns token with enriched scopeTags when present', async () => {
    ...
    expect(body.data.scopeTags[0]).toMatchObject({
      id: 'tag-001',
      name: 'backend',
      color: '#ff0000',
    });
