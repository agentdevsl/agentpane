...
describe('IT-013: Task move to in_progress without sandbox', () => {
  beforeEach(async () => {
  ...
  it('does not trigger agent when containerAgentService is set but sandbox is not enabled', async () => {
    const db = getTestDb();
    const project = await createTestProject();
    const task = await createTestTask(project.id, { column: 'backlog' });
  ...
  it('triggers agent when containerAgentService is set and sandbox.defaults is enabled', async () => {
    const db = getTestDb();
    const project = await createTestProject();
    const task = await createTestTask(project.id, { column: 'backlog' });
  ...
  it('sets startedAt when moving to in_progress', async () => {
    ...
    if (result.ok) {
      expect(result.value.task.startedAt).toBeTruthy();
      expect(typeof result.value.task.startedAt).toBe('string');
    }
  });
});
