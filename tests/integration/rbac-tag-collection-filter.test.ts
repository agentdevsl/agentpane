...
async function seedFixtures(db: ReturnType<typeof getTestDb>): Promise<Fixtures> {
  ...
  // Tags are scoped to project folders. The legacy `team_id` column was
  // dropped by v43 migration; only `project_folder_id` remains. The team
  // insert is still required for other test setup in this suite.
  execRawSql(
